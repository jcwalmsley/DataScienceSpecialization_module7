...
g
```
###^^^^^^^^####  Comment on the sinusoidal pattern

```{r ResidFit, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
y <- mtcars$mpg
x <- (mtcars$cyl + mtcars$disp + mtcars$wt + mtcars$am==0 + mtcars$am==1)
n <- length(y)
...
dfbetaPlots(lm(mpg ~ factor(am) + cyl + disp + wt, data = mtcars, family = binomial))
...
fbf3 <- lm(mpg ~ cyl + disp + factor(am) + wt, data = mtcars)
scf3 <- coef(summary(fbf3))
scf3
scf3[1,1] + c(-1, 1) * qt(.975, df = fbf3$df) * scf3[1, 2]
scf3[2,1] + c(-1, 1) * qt(.975, df = fbf3$df) * scf3[2, 2]##****
scf3[3,1] + c(-1, 1) * qt(.975, df = fbf3$df) * scf3[2, 2]##****
scf3[4,1] + c(-1, 1) * qt(.975, df = fbf3$df) * scf3[2, 2]##****
```
