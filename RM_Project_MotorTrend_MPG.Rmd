...
## Histograms
---
```{r Hist Figure 1, echo=FALSE, fig.height=6.5, fig.width=6.5}
...
```{r Box Plot Figure 1, echo=FALSE, fig.height=5.5, fig.width=5.5}
data("mtcars")
data("mtcars")
mtcars$vs <- factor(mtcars$vs, labels = c("V-block", "S-block")); mtcars$am <- factor(mtcars$am, labels = c("Auto", "Manual")); mtcars$gear <- factor(mtcars$gear); mtcars$carb <- factor(mtcars$carb)
head(mtcars,3)
...
```{r MultvariateLM(all vars)b, echo=FALSE, fig.height=6.5, fig.width=6.5}
par(mfrow = c(2, 2), mar = c(4, 5, 2, 1))
