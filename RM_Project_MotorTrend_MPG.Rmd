...
```{r ProcessData, message=TRUE, warning=TRUE, include=FALSE, results='hide'}
data("mtcars")
for(i in c(2,8:11))mtcars[,i] <- as.factor(mtcars[,i])
str(mtcars)
```

# VII.         Descriptive Statistics (view first & last three rows & factor variables 2 & 8:11)
```{r DescriptiveStats, echo=TRUE, message=TRUE, warning=TRUE, results='hide'}
...
```{r Fig.6_MVLM(allvars), echo=FALSE, fig.height=4, fig.width=4, message=FALSE, warning=FALSE, results='hide'}
...
```{r Fig.7_MVLM(allvars)A, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r Fig.8_BestFitModeling, echo=TRUE, fig.height=5, fig.width=5, message=TRUE, warning=TRUE, results='hide'}
library(car)
data("mtcars")
fbf1 <- lm(mpg ~ hp + drat + am, data = mtcars)
vif(fbf1)
sqrt(vif(fbf1))
library(ggplot2)
par(mfrow = c(1,1), mar = c(4,4,2,2)) # set margin
g <- ggplot(mtcars, aes(x = hp + drat +  wt + am, y = mpg),)
g <- g + xlab(">hp + drat + am<")
g <- g + ylab("Miles per Gallon")
g <- g + geom_point(size = 3.0, col = "blue", alpha = 0.55)
g <- g + geom_smooth(method = "lm", col = "black")
...
# DIAGNOSTICS A
```{r Fig.9_ResidFit, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
y <- mtcars$mpg
x <- (mtcars$hp + mtcars$drat + mtcars$am)
n <- length(y)
...
# DIAGNOSTICS B
```{r Fig.10_dfbetaPlots, echo=TRUE, fig.height=5, fig.width=5}
dfbetaPlots(lm(mpg ~ hp + drat + am, data = mtcars, family = binomial))
...
```{r ConfInt, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Fig.11_Predict, echo=FALSE, fig.height=5, fig.width=6}
