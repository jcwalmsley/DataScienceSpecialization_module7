...
# III. Analysis Considerations:
        Descriptive; Exploratory; Regression to the mean, simple linear regression  
        Multivariable regression; Adjustments; Residuals, variation, diagnostics
        Multiple variables & model selection; GLMs & Binary GLMs
...
## Pairs
```{r Pairs Figure 1, echo=TRUE, fig.height=6, fig.width=6}
...
mlr1 <- lm(mpg ~ ., data = mtcars);coef(summary(mlr1));plot(mlr1)
```
...
par(mfrow = c(2, 2), mar = c(4, 5, 2, 1))
mlr1 <- lm(mpg ~ . -1, data = mtcars);coef(summary(mlr1));plot(mlr1)
```

## Multivariate LM Nested Plot
```{r Nested, echo=TRUE, results='hide', fig.height=6, fig.width=6}
data("mtcars")
names(mtcars)
