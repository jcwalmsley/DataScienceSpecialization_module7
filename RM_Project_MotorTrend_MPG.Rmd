...
# Processing the data:
        Transformations;  
                ...
                2 change variable labels in columns 8 & 9;      
                        ...
                        b Note; for column header 9 = am; variable names = A-type = A, & M-typr;  
```{r, Process data, echo=TRUE}
data("mtcars")
mtcars$vs <- factor(mtcars$vs, labels = c("V-block", "S-block")); mtcars$am <- factor(mtcars$am, labels = c("A-type", "M-type")); mtcars$gear <- factor(mtcars$gear); mtcars$carb <- factor(mtcars$carb)
